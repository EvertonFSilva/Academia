...
public class RepositorioProfessorSQL implements ServicoRepositorioProfessor {
	...
	public boolean inserirProfessor(EntidadeProfessor professor, EntidadeEndereco endereco) {
		...
		if (enderecoId != -1) {
			try {
				OperacoesBancoDeDados.inserir(
						"INSERT INTO Professores (cpf, nome, salario, especialidade, turno, endereco_id) VALUES (?, ?, ?, ?, ?, ?)",
						professor.obterCPF(), professor.obterNome(), professor.obterSalario(),
						professor.obterEspecialidade(), professor.obterTurno(), enderecoId);
				return true;
	...
	public EntidadeProfessor buscarProfessorPorCPF(String cpf) {
		try {
			...
			if (resultSet.next()) {
				...
				EntidadeEndereco endereco = this.repositorioEndereco
						.buscarEnderecoPorId(resultSet.getInt("endereco_id"));
	...
	public EntidadeProfessor buscarProfessorPorId(int professorId) {
		try {
			ResultSet resultSet = OperacoesBancoDeDados.selecionar("SELECT * FROM Professores WHERE id = ?",
					professorId);
			if (resultSet.next()) {
